...
# parser.add_argument() 是 Python argparse 模块中的一个方法，用于定义程序可以接受的命令行参数。
...
def main():
	...
	# Global configuration of the datasets(数据集的全局配置)
	...
	##############################
	# Data loading code
	# TODO: For now only support 224x224 input size of 32x32. Need more work to support more resolutions(分辨率)
	if args.database == 'cifar-10':
		...
		if args.gcn:
			normalize = GCN()
			 ...
			 # TODO: Use the same normalization than CIFAR-10(CIFAR-10是一个广泛使用的计算机视觉数据集，主要用于图像分类任务的研究和基准测试。)
			 # TODO: That might be suboptimal...(这可能不太理想)
			if USE_COLOR:
				normalize = transforms.Normalize(mean=[x/255.0 for x in [125.3, 123.0, 113.9]],
												 std=[x/255.0 for x in [63.0, 62.1, 66.7]])
			else:
	...
	# Create model
	##################################
	# TODO: This condition only work if we have only two image size(这个条件只在我们只有两种图像尺寸时才起作用)
	big_input = INPUT_SIZE != 32
	# Only for scattering transform (as coefficient are precomptuted)(由于系数是预先计算的)
	scattering=None 
	if args.model == 'densenet':
		no_init_conv = args.no_init_conv
		if(INPUT_SIZE > 128):
			# For these input size, init conv(卷积) are necessary
			no_init_conv = False 
	...
	# get the number of model parameters获取模型参数的数量
